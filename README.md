...
To summarize `upload.changelog_path` before its Telegram document is sent, enable `telegram.changelog_summary` as shown in the example config. The `claude_cli` provider runs `claude -p --model haiku` with tools and session persistence disabled. It removes Anthropic API environment variables from the child process so the Claude CLI uses its signed-in subscription. If Claude’s structured JSON response exhausts its one-turn limit on a larger changelog, the runner records the structured error and retries once with a constrained plain-text prompt before giving up. Summary generation or delivery failure only logs a warning; it never blocks the original changelog document or turns a successful release into a failed one. Use `--test-changelog-summary changelog_tester.md --platform ios` to generate and send only the summary.
